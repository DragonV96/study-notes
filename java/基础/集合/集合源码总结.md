...
- 扩容大小是原来容量的**1.5倍**
- 数组长度最大值为 **`Integer.MAX_VALUE - 8`** 即 2^32 - 8 （因为一些虚拟机会在数组中保留一些头部字段header words，所以即便没有达到理论的最大数组长度，也有可能因 VM 限制而导致分配失败，抛出 `OutOfMemoryError`）
- 新增时允许 null 值
...
答：ArrayList 有最大容量的，为 Integer 的最大值 - 8，大于这个值 JVM 是不会为数组分配内存空间的，LinkedList 底层是双向链表，理论上可以无限大。但源码中，LinkedList 实际大小用的是 int 类型，这也说明了 LinkedList 不能超过 Integer 的最大值，不然会溢出。
