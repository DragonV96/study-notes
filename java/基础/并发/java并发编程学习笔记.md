...
Java 线程内存模型将确保所有线程查询 volatile 修饰的变量的值是一致的。
...
- JMM 提供一致的内存可见性保证
  ...
  - 对于处理器，JMM 要求 Java 编译器在生成指令序列时，插入特定类型的内存屏障（Memory Barriers）指令，来禁止特定类型的处理器重排序
...
- 监视器规则：一个锁的解锁，happens-before 于随后对这个锁的加锁
- volatile变量规则：一个volatile 域的写，happens-before 于人随后对这个 volatile 域的读
