...
- 在 Executor 已经关闭或对最大线程和最大队列都使用饱和时，可以使用 RejectedExecutionHandler 类进行异常捕捉，有如下四种处理策略：ThreadPoolExecutor.AbortPolicy（中止策略）、ThreadPoolExecutor.DiscardPolicy（丢弃策略）、ThreadPoolExecutor.CallerRunsPolicy（放弃最旧的策略）、ThreadPoolExecutor.DiscardOldestPolicy（调用方执行的策略）；
