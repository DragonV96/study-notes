...
如果程序运行的全部代码（包括自己编写的、第三方包中的、从外部加载的、动态生成的等所有代码）都已经被反复使用和验证过，则可以使用 `-Xverify：none` e参数来关闭大部分的类验证措施，以缩短虚拟机类加载的时间。
...
- 调用限定符：CONSTANT_Dyna-mic_info 和 CONSTANT_InvokeDynamic_info
...
双亲委派模型要求除了顶层的启动类加载器外，其余的类加载器都应有自己的父类加载器。3）
