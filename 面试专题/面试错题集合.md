...
答：TCP 拥塞控制分为 **四个阶段性策略**，每一阶段都有对应的算法：

| 阶段 | 策略                             | 作用                             |
| ---- | -------------------------------- | -------------------------------- |
| ①    | 慢启动（Slow Start）             | 快速探测可用带宽，指数级增长窗口 |
| ②    | 拥塞避免（Congestion Avoidance） | 线性增长窗口，避免过快拥塞       |
| ③    | 拥塞发生（Fast Retransmit）      | 快速检测丢包，不等超时           |
| ④    | 快速恢复（Fast Recovery）        | 快速收敛窗口，恢复传输速度       |

核心参数：

- **cwnd（拥塞窗口）**：TCP 自己维护的窗口，限制发送速率。
- **ssthresh（慢启动阈值）**：判断何时从“慢启动”切换到“拥塞避免”。
- **rwnd（接收窗口）**：接收方告知的能力（流量控制）。
...
- 慢启动（Slow Start）：
    ...
    - 当 `cwnd >= ssthresh`，切换到“拥塞避免”阶段。
    - **作用**：快速探测网络带宽。
- 拥塞避免（Congestion Avoidance）：
    - 每个 RTT，`cwnd += 1`，线性增长。
    - 更加温和，避免因暴涨引发拥塞。
- 拥塞发生（丢包） - Fast Retransmit：
    - 若连续收到 3 个重复 ACK，立即重传丢失的包（无需等超时）。
    - 推测网络拥塞开始。
- 快速恢复（Fast Recovery）：
    - 拿到重复 ACK 后：
        - `ssthresh = cwnd / 2`
...
- Cookie Token + 共享 Session 存储（最常见）
- JWT + 无状态认证（更偏向无 Session）
- Session 粘性（Nginx IP Hash）
- Spring Session + Redis（Spring生态推荐方案）
...
​	2.【不合格】你知道哪些分布式事务方案？分别有什么特点？适合什么场景？

答：5种方案：

| 类型                           | 技术方案                 | 适用场景                         | 特点                              |
| ------------------------------ | ------------------------ | -------------------------------- | --------------------------------- |
| XA 规范（两阶段提交）          | Atomikos、Narayana       | 同一事务跨多个数据库资源（JTA）  | 强一致性，性能差，耦合重          |
| TCC 模式（Try-Confirm-Cancel） | Hmily、ByteTCC           | 金融、电商订单支付、红包扣款等   | 业务侵入性强，可控性高            |
| 本地消息表 + 状态机            | 自实现                   | 微服务拆分订单状态流转等         | 最灵活、稳定，但开发复杂          |
| 可靠消息最终一致性             | RocketMQ 消息事务、Seata | 异步通知、支付回调、账务入账     | 最终一致性，容忍短暂不一致        |
| Seata AT 模式                  | Seata（阿里开源）        | MySQL + 微服务场景下的分布式更新 | 自动代理 JDBC，侵入性低，使用广泛 |

| 事务模型          | 业务强一致性 | 性能 | 成熟度 | 实现复杂度 | 推荐使用场景             |
| ----------------- | ------------ | ---- | ------ | ---------- | ------------------------ |
| XA                | ✅ 高         | ❌ 低 | ✅ 成熟 | ❌ 高       | 内网、企业级财务系统     |
| TCC               | ✅ 高         | ✅ 高 | ✅ 成熟 | ❌ 较高     | 金融、交易所、支付系统   |
| 本地消息表+状态机 | ✅ 可控       | ✅ 高 | ✅ 灵活 | ❌ 高       | DEX 与链下联动、复杂补偿 |
| 消息最终一致性    | ⚠ 最终一致   | ✅ 高 | ✅ 高   | ✅ 中等     | 电商订单、短信通知       |
| Seata AT          | ✅ 高         | ✅ 中 | ✅ 成熟 | ✅ 低       | 微服务数据库更新场景     |
...
​	2.【不合格】Redis 分布式锁存在哪些问题？如何优化？

答：如下：

| 问题                     | 原因与后果                   | 优化方式                             |
| ------------------------ | ---------------------------- | ------------------------------------ |
| 非原子释放（先get再del） | 并发时容易误删其他线程的锁   | 使用 Lua 脚本原子校验 value 并删除   |
| 客户端崩溃锁未释放       | key 永远不会删除             | 设置过期时间 + Watchdog 自动续约机制 |
| 锁失效时间不合理         | 时间过长影响吞吐，过短易误删 | 设置合理 TTL 或 Watchdog             |
| 无重入、无阻塞、无公平性 | 多线程并发时功能不完备       | 需要 Redisson 等高级框架支持         |



​	3.【不合格】Redisson 实现分布式锁的核心原理是什么？

答：Redisson 分布式锁底层是 Redis 实现，核心机制：

- 加锁时：使用 Lua 脚本 + UUID 标识唯一线程
- 锁续期：看门狗机制，每隔 `10s` 自动续期，防止长时间业务中断锁释放
- 可重入锁：在一个线程中多次加锁，通过 Hash 结构记录次数
- 公平锁：维护等待队列，先进先出保证公平性
...
- 设置过期时间（避免永远不释放）
- 使用看门狗机制自动续约（Redisson）
- 解锁时必须校验是否是自己的锁（避免误删）
- 必要时设置锁抢占超时时间 + 回退策略（失败立即重试/延迟重试）
...
​	7.【不合格】MySQL 哪些场景下索引会生效？哪些场景下索引不会生效？
...
- 隐式类型转换
- 联合索引未命中最左前缀
- 负向查询（即 `!=` 和 `IS NOT NULL`，一般索引不生效）

> 对 NULL 使用 IS NULL 可走索引，但 NOT NULL 不行。
...
- 覆盖索引
- IN 查询（有限集合）
- ORDER BY + LIMIT 命中索引（避免 filesort）
...
答：**MySQL 错误日志（Error Log）** 是排查数据库故障、启动问题、主从复制异常、崩溃恢复等问题时最核心的日志之一，常见记录内容如下：

| 记录内容       | 示例                          |
| -------------- | ----------------------------- |
| 启动与关闭日志 | MySQL 启动、关闭、重启过程    |
| 错误消息       | 启动失败、配置错误、表损坏等  |
| 主从复制错误   | IO 线程、SQL 线程报错         |
| 崩溃恢复信息   | crash-recovery 检测与自动恢复 |
| InnoDB 异常    | redo log 加载失败、死锁等     |
...
| 特性              | B+ 树索引                        | Hash 索引                        |
| ----------------- | -------------------------------- | -------------------------------- |
| 底层结构          | 多路平衡树（多叉树）             | 哈希表（key 经过 hash 函数映射） |
| 查询方式          | 范围查找、范围扫描（左匹配）     | 精确匹配 key（= 查询）           |
| 排序性            | 有序（适合范围、多列排序）       | 无序（不支持 range 查询）        |
| 支持范围查询      | ✅ 是                             | ❌ 否                             |
| 支持模糊匹配 like | ✅（前缀）                        | ❌ 不支持模糊匹配                 |
| 回表操作          | 可能回表（非覆盖索引）           | 一般需回表                       |
| 支持联合索引      | ✅ 是（左匹配原则）               | ❌ 不支持                         |
| 使用场景          | 大多数情况都适合（默认）         | 内存型表或高并发精确查找         |
| 引擎支持          | InnoDB/MyISAM（默认使用 B+Tree） | Memory（HASH）                   |

B+ 树适合 99% 业务查询，支持排序、范围、前缀匹配、联合索引；Hash 仅适合少量精确查询场景，无法进行范围查询。
...
- InnoDB log（InnoDB 事务日志）：Redo Log + Undo Log
    - redo log（重做日志）：用于保证事务的 **持久性**（crash-safe），保证宕机时数据不丢失，减少磁盘 IO 提升数据库性能；
    - undo log（回滚日志）：保证事务的 **原子性 & MVCC**，回滚时，提供历史数据，或用于快照读（Consistent Read）；
- bin log（归档日志）：记录 **所有更改了数据库数据的操作（写操作）**，是主从复制、数据恢复的关键组件，保证数据一致性；
- error log（错误日志）：排查数据库故障、启动问题、主从复制异常、崩溃恢复等问题的错误信息和警告，排查故障首选日志；
- general log（通用查询日志）：记录 **所有客户端发送的 SQL 语句**（不管是否执行成功），用于 **审计、调试、分析问题**；
- slow query log（慢查询日志）：记录 **执行时间超过设定阈值的 SQL 语句**，是 MySQL 性能优化的核心工具之一。

MySQL 日志对比表

| 日志类型     | 作用                             | 典型用途                 | 是否默认开启 |
| ------------ | -------------------------------- | ------------------------ | ------------ |
| 错误日志     | 记录服务运行异常、启动崩溃等信息 | 故障排查、服务状态监控   | ✅ 是         |
| 通用查询日志 | 记录所有请求 SQL 语句            | 审计、调试问题、日志分析 | ❌ 否         |
| 慢查询日志   | 记录执行慢的 SQL                 | SQL 优化、性能瓶颈分析   | ❌ 否         |
| 二进制日志   | 记录所有数据变更操作             | 主从复制、数据恢复       | ✅ 是         |
| 事务日志     | 保证事务的原子性和持久性         | 崩溃恢复、MVCC一致性快照 | ✅ 是         |
...
②写入 bin log，提交事务，处于 commit 状态。



​	22.【遗忘】MySQL 是如何保证宕机时数据不丢失的？

答：MySQL（InnoDB 引擎）主要依靠 **WAL机制（Write-Ahead Logging）**，即“先写日志、再写数据”，以及 **Redo Log + Binlog 的双重保障机制**，来保证**宕机后数据不丢失、事务一致性不被破坏**。

- **Redo Log**：用于崩溃恢复，保证数据持久化
- **Binlog**：用于主从复制和恢复到历史某个时间点（归档日志）



​	23.【遗忘】说一下 MySQL 主从同步原理。

答：MySQL 的主从复制本质上是 **基于二进制日志（Binlog）的异步数据复制机制**：

①主库写 Binlog；

- 主库（Master）上执行写操作，如 `INSERT / UPDATE / DELETE`
- 写入 **Binlog（二进制日志）**，Binlog 以 event 的形式记录下所有变更

②从库 IO 线程拉取 bin log；

- 从库（Slave）启动 **IO 线程**
- 向主库发起 `dump` 请求，持续拉取 Binlog（新产生的）

③从库 SQL 线程重放 bin log

- 从库将 bin log 存入 **本地 relay log（中继日志）**
- 启动 **SQL 线程**，读取 relay log，并执行其中的 SQL 事件，实现数据同步



​	24.【遗忘】说一下UPDATE语句的执行流程。
...
⑤执行 update 语句时的内部流程（以 `UPDATE t SET c=3 WHERE id=2` 为例）：

- 取 ID=2这一行
- 判断数据页是否在内存中？在则返回行数据，不在则从磁盘中读入内存返回行数据
- 将字段c设置为3
- 写入新行
- 新行更新到内存
- 写入 redo log 进入prepare 阶段，
- 写入 bin log 
- 提交事务，处于commit 阶段，InnoDB 空闲时记录刷盘。
...
答：MySQL 的锁主要可以分为以下几个维度：

| 维度           | 锁类型                                 | 描述                                      |
| -------------- | -------------------------------------- | ----------------------------------------- |
| **按粒度分类** | 全局锁、库级锁、表级锁、页级锁、行级锁 | 控制锁定范围，从整个库实例到表、页甚至行  |
| **按操作分类** | 读锁（共享锁）、写锁（排他锁）         | 读写是否互斥，是否允许并发读写            |
| **按实现方式** | 意向锁、间隙锁、记录锁、临键锁         | InnoDB 特有，用于并发事务中精细控制锁粒度 |
| **特殊机制**   | 自增长锁、元数据锁、死锁检测等         | 特殊操作或状态下产生的锁                  |

**按粒度分类**

| 锁粒度     | 具体类型               | 作用范围             | 应用场景举例                        |
| ---------- | ---------------------- | -------------------- | ----------------------------------- |
| **全局锁** | 全局读锁（FTWRL）      | 整个数据库实例       | 全库备份、逻辑导出（`mysqldump`）等 |
| **库级锁** | 数据库锁（极少见）     | 某个数据库（很少用） | 一些特殊权限或管理操作              |
| **表级锁** | 表锁、元数据锁（MDL）  | 整个表               | DDL、MyISAM、MDL冲突等              |
| **页级锁** | B+树页级锁（极少见）   | 索引的某个页         | BDB 引擎等（InnoDB 不使用）         |
| **行级锁** | 记录锁、间隙锁、临键锁 | 某一条或多条记录     | InnoDB 精细控制并发访问             |

**按操作分类**

| 类型        | 允许并发？ | 会阻塞写？ | 举例                                    |
| ----------- | ---------- | ---------- | --------------------------------------- |
| 共享锁（S） | ✅ 可多个   | ✅ 会阻塞写 | `SELECT * FROM user LOCK IN SHARE MODE` |
| 排他锁（X） | ❌ 不可并发 | ✅ 阻塞所有 | `SELECT * FROM user FOR UPDATE`         |
...
- 意向锁（Intention Lock）
    ...
    - **底层逻辑**：表级的逻辑锁，辅助判断是否能加表锁；
    - **举例**：事务要给一行加 X 锁，需先在表上加 IX 锁。

- 间隙锁（Gap Lock）

    - **作用**：锁住索引中的间隙，防止幻读；

        **举例**：`SELECT * FROM t WHERE id > 10 FOR UPDATE;`
         会锁住 (10, +∞) 区间的索引空间；

        **场景**：可重复读隔离级别 + 范围查询，防止新插入数据。

- 记录锁（Record Lock）

    - **作用**：锁住某一行数据（主键或唯一索引）；
    - **特性**：粒度小，并发性好，**存在于唯一索引或主键上才是纯行锁，否则可能退化为临键锁/间隙锁**；
    - **使用场景**：事务中对特定记录的精确更新；
    - **举例**：`SELECT * FROM user WHERE id = 1 FOR UPDATE;`

- 临键锁（Next-Key Lock）

    - **作用**：记录锁 + 间隙锁的组合，锁住当前行以及前后的间隙；

    - **场景**：解决幻读的核心机制；

    - **举例**：`SELECT * FROM user WHERE age = 30 FOR UPDATE;` 

        > 如果 age=30 存在，会锁住 (29,30] U [30,31) 这个范围

**特殊机制**

- 自增长锁（AUTO-INC Lock）
    - **作用**：保护 `AUTO_INCREMENT` 字段生成逻辑；
    - **机制**：一种表级锁；
    - **场景**：大量并发插入自增字段数据时的冲突热点；
    - **优化方案**：
        - 使用 `innodb_autoinc_lock_mode = 2`（**轻量级锁**）。
- 元数据锁（Metadata Lock, MDL）
    - **作用**：保证表结构读取/修改时一致性；
    - **现象**：查询时无法 `ALTER TABLE`；
    - **项目场景**：某个慢查询长事务导致建表语句阻塞；
    - **解决方案**：
        - 控制事务时间，避免长事务；
        - 开启 `performance_schema` 查找元数据锁阻塞源。



​	26.【不合格】InnoDB 中有哪些锁？分别有什么作用？
...
| 间隙锁 | 行级   | 锁定某个值之间的“范围”     |
| 临键锁 | 行级   | 行锁 + 间隙锁的组合        |
| 自增锁 | 表级   | 锁住 `AUTO_INCREMENT` 的值 |

**按用途分类**

| 锁类型     | 功能作用         | 说明                     |
| ---------- | ---------------- | ------------------------ |
| 共享锁 (S) | 可读不可写       | 多事务可并发读取         |
| 排他锁 (X) | 可读可写         | 阻塞其他事务读写         |
| 意向锁     | 加速表锁冲突判断 | 表级锁，声明行级锁意图   |
| 插入意向锁 | 插入时加的间隙锁 | 多个插入可并发（非冲突） |

插入意向锁（Insert Intention Lock）

- **作用**：多个事务并发在不同间隙插入时不会冲突
- **发生时机**：插入新记录时由 InnoDB 自动添加
- **用于避免不必要的死锁**
...
| 维度                | 主键索引（Primary Key）                              | 唯一索引（Unique Index）                   |
| ------------------- | ---------------------------------------------------- | ------------------------------------------ |
| **是否允许为 NULL** | ❌ 不允许为 NULL                                      | ✅ 允许为 NULL（可多个 NULL）               |
| **每张表数量限制**  | 只能有一个主键                                       | 可以有多个唯一索引                         |
| **默认是否聚簇**    | ✅ InnoDB 默认是聚簇索引（clustered index）           | ❌ 默认是非聚簇索引（secondary index）      |
| **设计意图**        | 强制表的唯一性标识，记录的唯一身份                   | 用于保证某字段值的唯一性，可能不用于主标识 |
| **语法形式**        | `PRIMARY KEY (id)`                                   | `UNIQUE KEY uk_username (username)`        |
| **本质区别**        | 是一种主约束，决定了表的物理组织方式（在 InnoDB 中） | 是一种附加约束，不决定数据组织方式         |



​	28.【不合格】主鍵索引就是聚簇索引吗？

答：主键索引：又叫聚簇索引（clustered index）



​	29.【不合格】如何通过 `EXPLAIN` 分析 SQL 执行计划？

答：使用 `EXPLAIN SELECT...`，查看关键字段：

| 字段          | 说明                          |
| ------------- | ----------------------------- |
| id   | 查询中每个 SELECT 子句或操作的唯一标识，id 越大优先级越高 |
| select_type   | 表示每部分 SELECT 类型        |
| table   | 当前这一步操作访问的表（或者派生表名、临时表名等） |
| partitions   | 查询命中的分区（分区表专用） |
| **type （重点）**      | 表示连接类型（性能从好到差：system > const > eq_ref > ref > range > index > all） |
| possible_keys | 可能使用的索引（值为 NULL 表示没有索引可以使用） |
| **key**       | 实际使用的索引                |
| key_len          | 使用的索引字节长度，用于判断是否“最左前缀索引”被完全使用                  |
| ref          | 表示这个索引是与哪一列或常量进行比较的                    |
| **rows**      | 预估扫描的行数（越小越好，越大说明效率越差）   |
| filtered          | 返回结果的条件过滤比例（百分比）         |
| **Extra（重点）**     | 是否使用临时表或排序等        |

**select_type** ：避免 DEPENDENT 类型的子查询，可以尝试改写为 JOIN。
...
| ----------- | -------------------------------------- |
| system      | 表只有一行（系统表）                   |
| const       | 主键/唯一索引 + 常量查询，最多返回一行 |
| eq_ref      | 唯一索引连接（多表连接中常见）         |
| ref         | 非唯一索引连接                         |
| fulltext    | 使用全文索引                           |
| ref_or_null | 使用索引查找，包含 NULL 判断           |
| index_merge | 使用多个索引合并                       |
| range       | 索引范围扫描（如 BETWEEN、IN）         |
| index       | 遍历整个索引（不回表）                 |
| ALL         | 全表扫描 🚨                             |

- 优化目标：`const` > `eq_ref` > `ref` > `range` > `index` > `ALL`
- `ALL` 最差，说明未命中索引。
...
| Using index condition | 使用 Index Condition Pushdown（MySQL 5.6+） |
| Using temporary       | 使用临时表（如 GROUP BY、排序等）           |
| Using filesort        | 需要额外排序 🚨                              |
| Using join buffer     | 使用连接缓冲区（如 BNL JOIN）               |
| Impossible WHERE      | WHERE 条件恒为 false                        |
| No matching index     | 没有匹配索引，可能导致全表扫描              |

- 避免 `Using temporary` + `Using filesort` 同时出现
- 追求 `Using index`，这是“覆盖索引”的表现



​	30.【不合格】碰到过数据库死锁吗？如何避免？

答：两个事务交叉等待对方持有的资源。

- **典型场景**：两个事务同时对不同记录加锁，但顺序不同
- **解决方式**：
    - 控制加锁顺序一致
    - 使用 `innodb_lock_wait_timeout`
...
②**序列化并 RLP 编码交易数据**（RLP = Recursive Length Prefix，但是不包含签名字段 v、r、s）

③对编码后的数据做 Keccak256 哈希，生成 32 字节消息摘要 hash

④用私钥做 ECDSA 签名，结果为 `(r, s, v)`，v 是恢复参数，取值一般为 27/28，或链 ID + 35/36（EIP-155）

⑤附上签名字段，RLP 编码整个交易（含 r、s、v），得到最终 raw transaction

⑥广播交易到以太坊网络



​	3【遗忘】EVM系列钱包的钱包签名流程是怎样的？

答：EVM 系列钱包在进行**消息或交易签名**时，整体流程主要基于以太坊的**椭圆曲线签名算法 secp256k1**。

签名算法包括：

- **椭圆曲线签名算法：ECDSA（secp256k1）**
- 哈希函数：**Keccak-256**

签名输入 & 输出：

- 输入：32 字节的消息哈希（message hash）
- 输出：`(r, s, v)` 三元组，签名结果总共 65 字节（其中 `v` 用于恢复公钥）
...
- 得到 `(r, s, v)` 三元组，最终为 65 字节签名

④**钱包返回签名（通常为 Base64 / Hex 编码）**
