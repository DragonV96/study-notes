...
​	6.【盲区】线程池工厂有哪些线程池类型？分别用在什么场景？其线程池参数是什么？
...
- 功能不一样：Synchronized 只能实现非公平锁，Lock 可以实现公平锁和非公平锁；

- 性能不一样：资源竞争激励的情况下，Lock 性能比 synchronized 好，竞争不激励的情况下，synchronized 比 Lock 性能好（因为 synchronize 会根据锁的竞争情况，从偏向锁-->轻量级锁-->重量级锁升级，竞争激烈的情况下，容易产生大量重量级锁，大量消耗系统资源）；
- 锁机制不一样：synchronize 是在 JVM 层面实现的，系统会监控锁（monitor）的释放与否（enter 和 exit）。Lock 是由 JDK 实现，需要手动释放，在finally块中释放，可以采用非阻塞的方式获取锁；
...
> 启动 jar 包时通过设置 JVM 参数 -XX:+HeapDumpOnOutOfMemoryError，开启虚拟机内存溢出时自动 Dump 当前内存快照到文件。
...
| 授权模式                                        | 适用场景                             |
| ----------------------------------------------- | ------------------------------------ |
| 授权码模式（authorization code）                | 接口开放给外部第三方系统调用         |
| 简化模式（implicit）                            | 纯前端，没有后端的 Web 系统          |
| 密码模式（resource owner password credentials） | 企业内部完全可信任的系统             |
| 客户凭证模式（client credentials）              | 企业内部不同系统或服务后端之间的调用 |
